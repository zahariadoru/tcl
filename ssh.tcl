...
proc portcheck_scan_pub {nick uhost hand chan text} {
        ...
        if {$port == ""} {
                ...
                if {[catch {set sock [socket -async $host $port]} error]} {
                        putquick "PRIVMSG $chan :13C1onnection 13T1o14: $host \($port\) 13w1as 13r1efused."
                } else {
...
proc portcheck_scan_dcc {hand idx text} {
        ...
        if {$port == ""} {
                ...
                if {[catch {set sock [socket -async $host $port]} error]} {
                        putdcc $idx "[portcheck_dopre]13C1onnection 13T1o14: $host \($port\) 13w1as 13r1efused."
                } else {
...
proc portcheck_connected {idx sock host port timerid} {
        ...
        if {[set error [fconfigure $sock -error]] != ""} {
                close $sock
                putdcc $idx "[portcheck_dopre]13C1onnection 13T1o14: $host \($port\) 13F1ailed14. \([string totitle $error]\)"
        } else {
                fileevent $sock writable {}
                fileevent $sock readable [list portcheck_read $idx $sock $host $port]
                putdcc $idx "[portcheck_dopre]13C1onnection 13T1o14:1  $host 12 \($port\) 13A1ccepted14!"
        }
}
proc portcheck_timeout {idx sock host port} {
        close $sock
        putdcc $idx "[portcheck_dopre]13C1onnection 13T1o14: to $host \($port\) 13T1imed 13O1ut14."
}
proc portcheck_read {idx sock host port} {
        global portcheck_setting
        if {$portcheck_setting(read)} {
                if {[gets $sock read] == -1} {
                        putdcc $idx "[portcheck_dopre]EOF On 13Connection 13T1o14: $host \($port\). 13S1ocket 13C1losed."
                        close $sock
                } else {
...
proc portcheck_connected_pub {chan sock host port timerid} {
        ...
        if {[set error [fconfigure $sock -error]] != ""} {
                close $sock
                putquick "PRIVMSG $chan :13C1onnection 13T1o14: $host \($port\) 13F1ailed14. \([string totitle $error]\)"
        } else {
                fileevent $sock writable {}
                fileevent $sock readable [list portcheck_read_pub $chan $sock $host $port]
                putquick "PRIVMSG $chan :13C1onnection 13T1o14: $host \($port\) 13A1ccepted14!"
        }
}
proc portcheck_timeout_pub {chan sock host port} {
        close $sock
        putquick "PRIVMSG $chan :13C1onnection 13T1o14: $host \($port\) 13T1imed 13O1ut14."
}
...
proc portcheck_read_pub {sock} {
        ...
        if {!$portcheck_setting(read)} {
                ...
                putquick "PRIVMSG $chan :EOF On 13C1onnection 13T1o14: $host \($port\). 13S1ocket 13C1losed."
                close $sock
        }
}
